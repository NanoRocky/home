...
export const getHxhWeather = async () => {
  ...
  const weatherResult: WeatherInfo = {
    ...
    temperature: getTemperature(
      result.data.low || result.data.night.low,
      result.data.high || result.data.night.high,
    ),
    winddirection: result.data.fengxiang || result.data.night.fengxiang,
    windpower:
      !result.data.fengli ||
      result.data.fengli.trim() === t("components.weather.windPowerUnit") ||
      result.data.fengli.trim() === "级"
        ? result.data.night?.fengli || t("common.status.unknown")
        : result.data.fengli,
  };
