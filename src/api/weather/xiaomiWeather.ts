...
export async function getXMWT() {
  ...
  weatherData.adCode = {
    city:
      ...
      i18n.global.t("console.weather.areaUnknown"),
  ...
  try {
    ...
    weatherData.weather = {
      ...
      windpower: convertWindSpeed(currentWeather.wind.speed.value, {
        returnRange: true,
...
export function convertWindSpeed(speed: number, options: WindConversionOptions = {}): string {
  const { returnRange = false, includeDescription = false } = options;
  const level = BEAUFORT_SCALE().find((l) => speed >= l.minSpeed && speed <= l.maxSpeed);
  if (!level) {
