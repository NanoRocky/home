...
const getGoogleCityName = async (lat: number, lng: number, key: string, languageCode: string) => {
    ...
    if (!res.ok || data.error || (data.status && data.status !== "OK" && data.status !== "ZERO_RESULTS")) {
        throw new Error(`Google Geocoding API Error: ${data.error?.message || data.status || res.status}`);
    }
    ...
    if (results && results.length > 0) {
        ...
        if (components) {
            ...
            return city || adminArea || results[0].formattedAddress || results[0].formatted_address?.split(',')[0];
        }
    }
    return null;
};
...
export const getGoogleWeather = async () => {
    ...
    try {
        ...
        weatherResult = {
            weather: current.weatherCondition?.description?.text || mapGoogleWeatherCode(current.weatherCondition?.type, t),
            temperature: Math.round(current.temperature?.degrees || 0),
            winddirection: current.wind?.direction?.cardinal ? mapWindDirection(current.wind.direction.cardinal, t) : t("common.status.unknown"),
            windpower: current.wind?.speed?.value != null ? `${kmhToBeaufort(current.wind.speed.value)}` : t("common.status.unknown"),
        };
    } catch (error) {
