...
export const getTencentWeather = async () => {
  ...
  if (!txskey) {
    ...
    adCodeResult = {
      city:
        adCode.result.ad_info.district ||
        adCode.result.ad_info.city ||
        adCode.result.ad_info.province ||
        t("components.weather.unknownRegion"),
    ...
    const txWeather = (await getTXWeatherS(
      txkey,
      adCodeResult.adcode,
      txskey,
    )) as TXWeatherResponse;
    if (String(txWeather.status) !== "0") {
      if (store.webSpeech) {
        stopSpeech();
        SpeechLocal("天气加载失败.mp3");
