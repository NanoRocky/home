  ...
  <div class="devsettings">
    <el-collapse class="collapse" v-model="activeName" accordion>
      <el-collapse-item :title="$t('components.settings.seasonalEffects.title')" name="1">
        <div class="item">
          <el-button
            plain
            class="el-button"
            :class="{ active: store.showSnowfall }"
            @click="toggleEffect('snow')"
            >{{ store.showSnowfall ? $t("common.status.disable") : $t("common.status.enable")
            }}{{ $t("components.settings.seasonalEffects.winterSnow") }}</el-button
          >
          <el-button
            plain
            class="el-button"
            :class="{ active: store.showFirefly }"
            @click="toggleEffect('firefly')"
            >{{ store.showFirefly ? $t("common.status.disable") : $t("common.status.enable")
            }}{{ $t("components.settings.seasonalEffects.autumnFirefly") }}</el-button
          >
          <el-button
            plain
            class="el-button"
            :class="{ active: store.showLantern }"
            @click="toggleEffect('lantern')"
            >{{ store.showLantern ? $t("common.status.disable") : $t("common.status.enable")
            }}{{ $t("components.settings.seasonalEffects.springLantern") }}</el-button
          >
        </div>
      </el-collapse-item>
          ...
          >
            <el-form-item
              ...
              :rules="[
                {
                  required: true,
                  message: $t('components.settings.wallpaper.idEmpty'),
                  trigger: 'blur',
                },
                {
                  pattern: /^\d+$/,
                  message: $t('components.settings.wallpaper.idNotNumber'),
                  trigger: ['blur', 'change'],
                },
              ]"
            >
      ...
      <el-collapse-item :title="$t('components.settings.wallpaper.advanced')" name="4">
        <div class="item">
          <span class="text">{{ $t("components.settings.wallpaper.autoSwitch") }}</span
          ><br /><br />
        ...
        <div class="item">
          ...
          >
            <el-radio :value="'auto'" border>{{
              $t("components.settings.system.autoLang")
            }}</el-radio>
      ...
      <el-collapse-item :title="$t('components.settings.system.weatherProvider')" name="6">
        <div class="item">
          <el-radio-group v-model="weatherProvider" size="small" text-color="#FFFFFF">
            ...
            <el-radio :value="'tencent'" border>{{
              $t("components.settings.system.providerTencent")
            }}</el-radio>
            <el-radio :value="'amap'" border>{{
              $t("components.settings.system.providerAMap")
            }}</el-radio>
            <el-radio :value="'google'" border>{{
              $t("components.settings.system.providerGoogle")
            }}</el-radio>
            <el-radio :value="'xiaomi'" border>{{
              $t("components.settings.system.providerXiaomi")
            }}</el-radio>
            <el-radio :value="'oioweb'" border>{{
              $t("components.settings.system.providerOioweb")
            }}</el-radio>
          </el-radio-group>
        </div>
      </el-collapse-item>
      <el-collapse-item :title="$t('common.action.reset')" name="7">
        <div class="item">
          <el-button plain class="el-button" @click="resetSettings()">{{
      ...
      <el-collapse-item :title="$t('components.settings.system.checkUpdate')" name="8">
        <div class="item">
          <div class="upver">
            {{ $t("app.info.version") }}v{{ versionInfo.version }}，{{ versTypeT }}，{{
              versionInfo.channel
            }}
            {{ $t("components.settings.system.channel") }}，by {{ versionInfo.upa }} 。
          </div>
        </div>
        <div class="item">
          <el-button plain class="el-button" @click="checkUpdate()">{{
            $t("components.settings.system.checkButton")
          }}</el-button>
        </div>
      </el-collapse-item>
    </el-collapse>
...
const checkUpdate = async () => {
  ...
  if (updinfo.status == "true") {
    ElMessage({
      message: t("components.settings.system.alreadyLatest", {
    ...
    ElMessage({
      message: t("components.settings.system.newVersion", {
        version: updinfo.latestVersion,
        type:
          updinfo.isPreview == "true" ? t("common.version.preview") : t("common.version.stable"),
      }),
      grouping: true,
    });
    if (store.webSpeech) {
